...
- **범주는 개인이 자유롭게 추가·삭제·이름 변경한다.** 고정된 5개 슬롯이 아니라, 사용자가 만들고
  지우는 목록이다.
- **색은 공지 작성 에디터(`RichTextEditor`)와 같은 팔레트에서만 고른다.** "테마 색상"(60색 그리드) +
  "표준 색상"(10색) — `frontend/src/components/RichTextEditor.tsx`의 `ColorPalette` 컴포넌트를
  그대로 재사용한다(색상 필터 기능(`ColorFilterManageModal`)과도 동일). 저장값은 `#RRGGBB` 형식의
  hex 문자열이다(백엔드 `PersonalMarkCategory.COLOR_HEX_RE`로 형식만 검증, 팔레트 소속 여부는
  검증하지 않는다).
...
| `color` | `#RRGGBB` 형식의 hex 문자열(테마 색상/표준 색상 팔레트 중 택1) |
...
- **새 범주의 기본 색**: "표준 색상" 10색(`RichTextEditor.EXCEL_STANDARD_COLORS`)을 범주 개수만큼
  순환 배정한다 — 11번째 범주부터는 색이 겹치기 시작한다(알려진 제한, §6). 색은 언제든
  ⚙ 범주 설정에서 테마 색상/표준 색상 팔레트 중 원하는 것으로 바꿀 수 있다.

## 6. 알려진 제한사항 / 확인이 필요한 것

- **색상 팔레트 소진**: 새 범주 기본색 자동 배정은 표준 색상 10가지를 순환한다. 범주를 11개 이상
  만들면 자동 배정 색이 겹친다. (사용자가 직접 테마 색상/표준 색상 팔레트에서 다른 색으로 바꿔
  회피할 수는 있다.)
