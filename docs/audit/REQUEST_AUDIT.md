...
- 심각도: ⚪경미 · 상태: ⏸️보류 (사용자 결정 2026-06-30 — 현재 동작상 문제 없어 보류)
- 결정/메모: 보류. (추후 견고화 시 make*Row의 sortOrder를 단조 증가 카운터/인덱스 기반으로 부여해 명시적 순번 보장. 동작 동일하나 취약성 제거.)

### [R-04] 연쇄 선택 옵션 조회 중복/과다 fetch (전 단계로 확장)  (관련: F-1.1, F-1.6, F-1.8, F-2.4)
...
- **확장(사용자 지시 2026-06-30):** 같은 "타이핑 매 글자 조회/디바운스 부재" 패턴이 뼈찜뿐 아니라 **Step1 라인→조합법→제품→조리법 체인, 흐름도, PRODC**까지 전반에 존재함을 전수 조사로 확인 → 전부 수정 대상에 포함.
- 심각도: ⚪경미(부하 관점) · 상태: ✅수정완료
- 결정/메모 (2026-06-30): **"정확 일치 + 시퀀스 토큰"** 방식 채택(디바운스 대신 — 정확성이 더 높다고 판단).
  - 공용 헬퍼 `matchedOrLoading`(값이 부모 옵션에 정확히 존재할 때만, 편집/투어 로드 중엔 우회) + `fetchOptions`(조회 키별 시퀀스 토큰으로 stale 응답 폐기 → 응답 순서가 꼬여도 항상 마지막 값 옵션만 반영).
  - 적용: ① 조합법→제품 ② 제품→조리법 ③ 조리법→J/O(유효할 때만 재생성, 부분 입력 중 비파괴) ④ 뼈찜 product→조리법 ⑤ 뼈찜 외부데이터(항목별 정확 일치 + (항목,값) 캐시 + 시퀀스 토큰, 토스트는 effect 결과 기반·중복 fetch 제거) ⑥~⑧ 흐름도 product/process_id/layer ⑨ PRODC 조합법→제품. 라인/원본위치/뼈찜·흐름도 location/PRODC 라인은 `<select>`라 미변경.
  - 토스트(나): "BB 데이터 N행/없음"을 외부데이터 effect 결과로, 조리법이 새로 유효해진 항목에 대해서만 1회 안내(별도 조회 없음).
  - 잔여(수용): 접두사가 유효 옵션인 경우(예 `"10"`·`"100"`) 옵션 조회가 2회 날 수 있으나 시퀀스 토큰으로 **결과는 항상 정확**. process_id의 JOB/OVL 재생성은 정확 일치 게이트만 적용(시퀀스 토큰 미적용) — prefix 충돌+순서꼬임이 겹치는 극히 드문 경우만 잔존, 필요 시 후속 보강.
  - 검증: `tsc --noEmit` 신규 에러 0. 커밋: `fix(request): R-04 옵션 조회를 '정확 일치 + 시퀀스 토큰'으로 전환(전 연쇄 단계)`.
...
- 심각도: ⚪경미 · 상태: 🟢의도된동작(정상) — 사용자 확정 2026-06-30
- 결정/메모: 삭제 시 존재하지 않는 탭 인덱스를 가리키지 않게 0으로 리셋하는 것이 안전하고, Step1 편집↔Step5 탭이 분리돼 실사용 영향이 없어 **의도된 동작으로 확정**. 재점검 제외. (R-04 수정으로 effect 재실행 빈도가 줄면 부수적으로 완화됨.)
...
- 심각도: 🟡보통 · 상태: ⏸️보류 (사용자 결정 2026-06-30 — 양 많고 데이터값/표시텍스트 구분 리스크로 나중에 일괄 진행)
- 결정/메모: 보류. 진행 시 계획 — DB 데이터 값(신규/적용/Yes 등)·주석 제외, 표시 텍스트만 `request.*` ko/en 동시 추가, 파일별 커밋.

### [R-07] catch 내 console.error  (관련: F-1.1, F-1.8, F-2.8, F-3.x)
- 위치: `index.tsx` 승인문서 로드 `.catch(console.error)`, 이미지 업로드/JOB FILE/OVL 조회 catch 3곳
- 재검토(2026-06-30): 규칙 I가 금지하는 건 `console.log`/`print()`(디버그 출력)이지 `console.error`(에러 로깅)가 아님. 4곳 모두 catch 내 에러 로깅이며 3곳은 사용자 toast와 함께, 281번은 실패 시 유일한 신호. **삭제하면 실패 원인(에러/스택) 진단 수단이 사라짐.**
- 심각도: ⚪경미 · 상태: 🟢의도된동작(정상) — 사용자 확정 2026-06-30 (유지)
- 결정/메모: 정상적인 에러 로깅으로 판단해 유지. 운영 콘솔 정리가 필요해지면 공통 logger 라우팅은 선택적 개선(결함 아님). 재점검 제외.

### [R-08] 잘못된 섹션 주석  (관련: F-1.6)
- 위치: `index.tsx` `handleFlowAddRow`와 `handleFlowDeleteRow` 사이의 `// ===== Date Format Helper =====`
- 증상: 흐름도 핸들러 위에 "Date Format Helper" 헤더가 잘못 남아 오도(실제 날짜 헬퍼는 `helpers.ts`). 리팩토링 잔재.
- 심각도: ⚪경미 · 상태: ✅수정완료
- 결정/메모 (2026-06-30): 잘못된 주석 1줄 삭제(동작 영향 0). tsc 신규 에러 0. 커밋 `fix(request): R-08 잘못된 섹션 주석 제거`.
...
### [R-12] flow_chart 옵션 Record 인덱스 키 → id 키 전환  (관련: F-1.6)
- 위치: `FlowProductOptions/FlowProcessIdOptions/FlowLayerIdOptions`(idx 키), Step1 옵션 읽기
- 재검토(2026-06-30): 지속 데이터 버그는 아니나, **R-01과 동일한 인덱스 키 취약 패턴**. 중간 흐름도 행 삭제 시 뒤 행 인덱스가 시프트되어 effect 재정렬 전까지 **드롭다운이 옛 행 옵션을 잠깐 보이거나 빈 채로 깜빡이고 불필요 재조회** 발생(최종엔 수렴).
- 심각도: ⚪경미 · 상태: ✅수정완료
- 결정/메모 (2026-06-30): flow 옵션 3종을 `Record<number>`→`Record<string>`(행 `row.id` 키)로 전환(R-01 bb_entries와 동일 방식). 효과·Step1 읽기 모두 `row.id` 기준 → 중간 행 삭제 시 깜빡임/오조회 제거. tsc 신규 에러 0.

### [R-13] MAP 검증 우회 + CLONE 원본 필수 누락 + 전환 초기화 버그  (관련: F-2.2, F-2.3, F-2.6~2.8)
- 위치: `validate(2)`; `handleMapTypeSelect`; `StepMap.tsx` 원본 블록
- 재검토(2026-06-30):
  - 등록형(EXISTING/CLONE)에서 지도편차·예외·C가문·X표시 검증 스킵은 해당 입력이 모두 `disabled`라 **일관됨(정상)**.
  - **실제 갭 ①:** CLONE은 원본 위치/Part ID가 입력 가능한데 `validate(2)`에 **필수 검증이 없어** 비워도 상신됨.
  - **실제 버그 ② (전환 초기화):** `handleMapTypeSelect`가 **CLONE/EXISTING으로 바꿀 때만** 초기화 → **NEW로 바꾸면 초기화 안 됨** → CLONE에서 채운 원본 등 StepMap 값이 잔존·저장됨.
- 심각도: 🟡→(②는 데이터 잔존) · 상태: ✅수정완료
- 결정/메모 (2026-06-30, 사용자 지시: CLONE 원본 필수화 + 전환 초기화 확인):
  - ① `validate(2)`에 `map_type==='CLONE'`이면 `source_line`·`source_partid` 필수 추가 + `StepMap` 원본 필드 에러 표시.
  - ② `handleMapTypeSelect`를 "이미 선택된 값이 있으면 어느 값으로 바꾸든 초기화 모달"로 변경 → NEW↔CLONE↔EXISTING 모든 전환에서 StepMap 필드 초기화(Step1/3/4/5 보존). 최초 선택은 모달 없음.
  - tsc 신규 에러 0. 커밋 `fix(request): R-13 ... R-17 ...`.
...
- 위치: `Step4.tsx` 좌측 원본 목록
- 증상: 행이 있으나 전부 매핑되면 tbody가 비어 안내 없이 빈 표만 보임.
- 심각도: ⚪경미 · 상태: 🟢의도된동작(정상) — 사용자 확정 2026-06-30
- 결정/메모: 상단 **"N행 조회됨" 카운터가 0**을 표시해 전부 매핑됐음을 확인 가능하므로 별도 안내 불필요. 수정 안 함, 재점검 제외.

### [R-16] 비활성 SD의 TBV/TLV 항목이 남아 상신됨  (관련: F-4.4, F-4.1)
- 위치: `detail.tbvtlv_entries`(추가 시점 SD 문자열 저장) vs O-layer 활성 SD; `index.tsx` 저장/동기화
- 증상(사용자 관찰): TBV/TLV 항목을 추가한 뒤 그 SD의 O-layer 행을 **비활성화해도 항목이 화면·상신 데이터에 그대로 남음**. draft SD도 SD가 사라져도 선택 유지돼 stale 추가 가능(엣지).
- 심각도: 🟡보통(데이터 정합성) · 상태: ✅수정완료
- 결정/메모 (2026-06-30, 사용자 결정: **영구 삭제**): `[oayerRows]` effect로 활성 TBV/TLV SD 집합을 계산해, 그에 속하지 않는 `tbvtlv_entries`(및 항목 내 SD)를 **영구 제거**하고 선택 중 draft SD도 정리. 비활성화 시 화면·저장 모두에서 사라짐(복원해도 되돌아오지 않음). 변경 없을 땐 no-op(무한루프/불필요 재렌더 방지). tsc 신규 에러 0.

### [R-17] bb_entries 부분 입력 허용 → 자동채움 빈 제품 옵션  (관련: F-1.8, F-5.2)
- 위치: `validate(1)` bb_entries 검증(기존 "최소 1개"); `Step4.tsx` 자동채움 범위 제품 select
- 증상: bb_entries가 "최소 1개만 채우면 통과"라 부분 입력 항목이 남을 수 있고, 그 빈 제품 항목이 Step5 자동채움 드롭다운에 노출돼 골라도 0행.
- 심각도: ⚪경미 · 상태: ✅수정완료
- 결정/메모 (2026-06-30, 사용자 지시: 추가 항목까지 모두 입력해야 진행): `validate(1)`를 **"모든 bb_entry 완전 입력(위치·제품·조리법)"** 필수로 강화(불필요하면 삭제 유도). Step1을 통과하려면 빈 항목이 없으므로 **자동채움 빈 제품 옵션 문제도 자연 해소**(별도 필터 불요). tsc 신규 에러 0.
