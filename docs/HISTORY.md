...
- ✅ **(2026-09) 상신일 시간 표시** — '상신일' 칸은 날짜만이 아니라 `utils/date.formatDateTime`으로
  `YYYY. M. D. HH:mm` 한 줄에 시:분까지 함께 보여준다(일반 탭 `doc.submitted_at`, 반려 탭 `snap.submitted_at`
  모두 동일). '결재 완료일'·'반려일' 칸은 기존과 동일하게 날짜만 표시한다(`formatDate`, 변경 없음).
