...
- 라인 항목은 `RequestPage/constants.ts` 의 `OPTION_LINE` 을 그대로 따른다. 여기에 라인이 추가되면
  탭도 자동으로 늘어나지만, 라벨 i18n 키(`history.filter_line_*`)는 함께 추가해야 한다.
- 라인 분류는 **제목 파싱이 아니라 원본값**(`detail.line`)을 본다. 제목 포맷이 바뀌어도 깨지지 않는다.
- **반려 문서는 반려 탭에만** 나온다. 전체·MY·라인 탭은 결재 완료 문서만 다루므로 서로 겹치지 않는다.
- 각 탭에는 결재 현황과 동일하게 건수 배지가 붙는다(0건이면 숫자 생략).
