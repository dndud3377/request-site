...
## 3-10. [4차] item_id 바코드 매칭을 n7material_spec 기반으로 재설계

- [ ] 백엔드: `/form-options/barcode/` 응답 label이 `n7barcode [날짜] [spec]` 형식(spec = n7material_spec의 '-' 뒤 문자), 키가 `spec`
- [ ] J-layer item_id 드롭다운 후보가 **step 숫자가 spec에 숫자경계로 일치**하는 것만 노출
      (예: step=1.0 → 15.0/1.05 후보는 제외, 1.0CDE/1.0 만)
