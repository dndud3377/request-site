...
| 항목 | 규칙 |
|---|---|
| 저장 | `UserProfile.mail_lines` (M2M → `Line` 마스터). 라인 '이름'으로 대조한다 |
...
| 기본값 | 사용자 생성 시 **활성 라인 전체**를 채운다(`models.assign_default_mail_lines`) — OIDC 최초 로그인·권한 관리 사용자 추가 경로 |
| 빈 집합 | '미설정'이 아니라 **'본인이 전부 껐다 = 메일 0통'** 을 뜻한다 |
...
- 구현: `mailer._filter_by_mail_lines()` → `mailer._apply_redirect(recipients, document)` 안에서
  **`MAIL_REDIRECT_TO` 강제보다 먼저** 수행된다. 필터로 수신자가 전부 빠지면 `MAIL_REDIRECT_TO`
  가 설정돼 있어도 발송하지 않는다.
- 변경 API: `PATCH /api/users/{id}/mail-lines/` body `{"lines": ["라인1", "라인3"]}` (전체 교체).
  **본인 행은 본인이, 그 외에는 MASTER 만** 바꿀 수 있다. 대상 역할이 아니면 400.
- 화면 동작: 라인 버튼을 누르면 즉시 저장되고 연한 파란색으로 켜진다. **해제할 때만 확인 모달**이 뜬다.
