...
| rejected | (반려) | 요청서 작성자 **+ 현재(최종) 회차에서 이미 합의했던 전원**(중복 제거). **PL 단계 반려(2026-07 추가)**: 다중 PL 지정 시, 같은 회차의 아직 합의/반려하지 않은 나머지 지정 PL(pending)도 포함 |
...
| `reject-step` (어느 단계든 반려, PL 제외) | ✅ | rejected: 작성자 + 현재 회차 기합의자 전원 |
