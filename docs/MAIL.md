...
- ✅ **(2026-08) 후결자 행 라벨 분리, (2026-09) 고정 후결자 라벨 변경**: 고정 후결자
  (`settings.POST_APPROVER_LOGINID`)는 RFG 팀 1명이라 단계명 **`RFG 후결`**(`POST_APPROVER_FIXED_LABEL`,
  2026-09 이전엔 `R`), PL 이 지정한 추가 후결자는 **`추가후결자`**(`POST_APPROVER_EXTRA_LABEL`)로
  표기한다. 웹 그리드의 두 칸(`후결자`→`RFG 후결` / `추가후결자`)과 같은 구분이다.
  ✅ **(2026-09) `R` → `RFG 후결`로 바꾸며 담당자 행과의 라벨 충돌도 해소됐다** — 예전엔 세로
  목록인 메일 카드에서 `R`(실제 R 담당자) 행과 고정 후결자 행의 라벨이 같아져 `R` 이 두 줄
  나왔다(웹 그리드에는 일반 경로에 R 담당자 칸이 없어 겹치지 않았지만 메일은 겹쳤다). 이제는
  라벨 자체로 구분되어 담당자 이름에 기대지 않아도 된다.
