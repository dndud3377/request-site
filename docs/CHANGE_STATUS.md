...
- **`updated`·`eqptype` 만 바뀐 경우도 기록하지 않는다** (2026-09 수정): 갱신시각만 올라간
  행은 아래 표의 5개 컬럼이 전부 같아 화면에서 **삭제·추가가 똑같아 보이는** 잡음이었다.
  이제 diff 판정에서 두 컬럼을 제외해 이력을 남기지 않는다. 단 **테이블 재적재는 그대로
  수행**되므로 의뢰서 작성 화면의 'Update 날짜'는 계속 최신값이다.
  자세한 배경은 `docs/SCHEDULER.md` "재적재 판정과 이력 diff 는 기준이 다르다" 절 참고.
