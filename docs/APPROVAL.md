...
- **표**: '현재 단계' 칸은 결재현황과 동일한 그리드(§3.3, 같은 `getDocTableRows`/`StageGrid`)를 쓴다.
  ...
  ✅ **(2026-09) 제품(조합법-제품-조리법) 칸 클릭 시 결재현황으로 이동하지 않고 그 의뢰 상세 모달을
  바로 연다** — `/approval?id=<문서id>`로 이동해 `ApprovalPage`의 기존 메일 딥링크(`?id=`, §3.6) 로직을
  재사용한다(문서 재조회 → `PagedDetailView` 모달 자동 오픈). 이 딥링크의 기존 동작대로 배경 목록의
  필터 탭은 초기화되고 검색창에 해당 문서 제목이 자동 입력된다(결재현황 화면 자체의 동작, 홈에서
  새로 만들지 않음).
