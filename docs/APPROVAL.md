...
핵심: **PL → R → (P[→검토자 PV] → J) ∥ (O, E[→검토자 EV] → 전원합의)**. R 합의 후 두 경로(path1=P→J, path2=O+E)가 **병렬** 진행된다.
P/E는 (2026-07부터) O/J와 동일하게 **검토중(claim) 방식**이며, 선점한 담당자가 다중 검토자(PV/EV)를 추가 지정할 수 있다.
지정 시 **담당자+검토자 전원 합의**가 끝나야 그 단계가 완료된 것으로 본다. **P**는 검토자 지정이 여전히 **선택**이라
검토자가 없으면 담당자 합의만으로 완료된다(하위호환). **E**는 (2026-07 하반기 변경) 담당자 합의 시 검토자(EV)
**1명 이상 지정이 필수**이며(Case K-3 참고), 미지정이면 합의 요청 자체가 400으로 거부된다 — 단 그 이전에 이미
검토자 없이 E 합의를 마친 진행 중 문서는 하위호환으로 그대로 완료 처리된다(Case G 참고).
...
### Case G — J / O / E 최종 합의 (`approve_step` agent in J/O/E/EV/RA, `views.py:509`)
...
- ✅ **E 검토중 + 다중 검토자(2026-07)**: E도 P와 동일하게 검토중 선점 후, `approve-step/`(agent='E')에
  ...
  최종 승인 판정 시 E는 **담당자+검토자(EV) 전원 합의**(`_stage_reviewers_complete`)를
  요구하며, 검토자가 없으면 담당자 합의만으로 충분하다(하위호환). EV도 **담당자(E) 합의 후에만** 처리 가능(순차 가드).
- ✅ **E 검토자 필수 게이트(2026-07 하반기)**: `approve_step`(agent='E')은 담당자 단계를 저장하기 전에
  `reviewer_loginids`가 전부 공백이고 해당 회차에 기존 `EV` step도 없으면 400 `{'error': '2차 검토자를 1명
  이상 지정해야 합니다.'}`로 거부한다(Case K-3 참고). **신규 E 합의는 검토자 없이는 더 이상 성립하지 않는다.**
  `agent='EV'`(검토자 본인의 합의)에는 이 게이트가 걸리지 않는다.
- ⚠️ **그런데도 `_stage_reviewers_complete`의 "검토자 0명이면 담당자 합의만으로 완료" 분기는 의도적으로 그대로
  남겨뒀다**: 위 게이트가 배포되기 이전에 이미 검토자 없이 E 담당자 합의를 마친 **진행 중 문서**들이 있고,
  그 문서의 E 단계는 이미 `approved`이며 `EV` step은 0개다. 만약 최종 승인 판정을 "EV 0개면 미완료"로 바꾸면
  그 문서들은 **검토자를 지정할 경로가 없어**(E 단계가 이미 approved라 재지정 불가) **영구히 승인되지 못한다**.
  따라서 판정 로직은 하위호환을 유지하며, 새 규칙(검토자 필수)은 **게이트 시점 이후의 신규 E 합의에만** 적용된다.
...
- ⚠️ **별도 지정 API 없음**: R 담당자 지정(`assign_step`이 담당자+검토자를 한 번에 받는 것)과 동일한 UX를
  ...
  `reviewer_loginids`(배열)를 함께 실어 보낸다. 이 요청 **한 번**으로
  검토자(PV/EV) pending step 생성과 담당자 단계 합의가 같이 처리된다(`_create_reviewers` → `step.action='approved'`
  저장 순서, 검증 실패 시 아무 것도 생성/변경되지 않음).
- ✅ **P는 선택, E는 필수(2026-07 하반기)**: `reviewer_loginids` 필드 자체는 P/E 공통으로 요청 payload 상
  선택(optional)이지만, **`agent='E'` 합의에는 서버가 별도 게이트를 건다** — 요청의 `reviewer_loginids`가
  전부 공백이고 해당 회차에 기존 `EV` step도 없으면 **`approve-step/`이 400
  `{'error': '2차 검토자를 1명 이상 지정해야 합니다.'}`**로 거부되어 담당자 합의 자체가 성립하지 않는다
  (`views.py` `approve_step`, `agent == 'E'` 분기). **P는 이 게이트가 없어 기존과 동일하게 검토자 없이도
  합의로 즉시 완료**된다(Case F 참고). `agent='EV'`(검토자 본인의 합의)에는 이 게이트가 걸리지 않는다.
  - ⚠️ **검증 위치가 어떤 쓰기보다 앞**: 이 게이트는 Validation System 화이트리스트 검사 직후,
    `_create_reviewers` 호출(검토자 step 생성)보다 **먼저** 실행된다. `@transaction.atomic`은 예외에만
    롤백하므로, 쓰기(검토자 생성/담당자 단계 저장) 이후에 400을 반환하면 그 쓰기가 커밋된 채 응답만
    실패하는 부분 커밋이 생기기 때문이다.
  - ⚠️ **운영 요건**: `_create_reviewers`는 담당자 본인을 검토자로 지정하는 것을 금지한다. 따라서
    **`TE_E` 역할 사용자가 2명 이상이어야** E 단계를 넘길 수 있다(1명뿐이면 검토자를 아무도 지정할 수
    없어 E 단계가 봉쇄된다). 의도된 2인 확인 제약이다.
...
- 화면: 검토중 상태에서 '합의' 버튼 옆에 R 담당자 지정과 동일한 스타일의 검토자 드롭다운이 노출된다.
  ...
  ✅ **E는 프론트도 게이트(2026-07 하반기)**: 검토자를 하나도 지정하지 않은 상태면 '합의' 버튼이
  **비활성화**되고 드롭다운 아래 안내 문구(`approval.reviewer_required_hint`, 지정 가능한 대상이 없으면
  `approval.reviewer_required_empty`)가 노출된다(`ApprovalPage.tsx`). '반려' 버튼은 그대로 활성 — 검토자
  없이도 반려는 가능하다. 백엔드 게이트가 최종 방어선이고 이 프론트 비활성화는 UX 보조일 뿐이다.
- 완료 조건: 담당자 + 지정된 검토자 **전원 합의**가 끝나야 해당 단계가 완료로 간주된다(Case F/G 참고,
  `_stage_reviewers_complete`). **P**는 검토자가 하나도 없으면 담당자 합의만으로 즉시 완료(하위호환).
  **E**는 위 게이트 때문에 신규 합의부터는 검토자가 반드시 1명 이상 존재하므로 사실상 항상 검토자 전원
  합의까지 기다린다 — 다만 **게이트 배포 이전에 이미 검토자 없이 합의를 마친 기존 문서의 E 단계는 예외**로
  남는다(Case G의 하위호환 분기 참고).
...
- **R 합의 후(병렬)**: 한 의뢰서를 **2행**으로 분리 표시 — path1(P/J), path2(O/E). rowSpan으로 좌측 공통열 병합.
  ...
  - path3(후결자 RA) 단계 텍스트도 동일하게, 완료 시 미표시가 아니라 **후결자 전원의 '라벨(이름)'을 나열**한다(2026-07 변경).
...
| 철회 | `withdraw/` | - |
| 합의 | `approve-step/` | `agent`, `comment`, `approver_name`, (P/E만) `reviewer_loginids`(배열 — 담당자 합의와 검토자 지정을 한 번에 처리. **P는 선택, E는 1명 이상 필수**(기존 EV 있으면 예외), Case K-3 참고) |
...
7. ✅ **(2026-07 해결) P·E 검토중(claim) 전환 + 다중 검토자(PV/EV)** — P·E도 지정하기 대신 검토중 방식으로 바꾸고,
   ...
   담당자+검토자 전원 합의가 끝나야 해당 단계가 완료로 간주된다(`_stage_reviewers_complete`).
...
8. ✅ **(2026-07 하반기 해결) MASK(E) 2차 검토자 필수화** — 위 7번의 "검토자 없으면 담당자 합의만으로 완료"는
   **P는 그대로**지만 **E는 더 이상 아니다**: `approve_step`(agent='E')이 담당자 단계를 저장하기 전에
   `reviewer_loginids`가 전부 공백이고 기존 `EV` step도 없으면 400으로 거부한다(검증은 어떤 쓰기보다
   먼저 실행 — 부분 커밋 방지). 최종 승인 판정(`_stage_reviewers_complete`)의 하위호환 분기 자체는
   **의도적으로 그대로 남겨서**, 이 배포 이전에 검토자 없이 이미 E 합의를 마친 진행 중 문서가 영구
   정지되지 않게 했다. 프론트도 검토자 미지정 시 '합의' 버튼을 비활성화한다(i18n
   `approval.reviewer_required_hint`/`reviewer_required_empty`). 상세는 Case K-3 / Case G 참고.
   ⚠️ **운영 요건**: 담당자 본인은 검토자로 지정할 수 없으므로 `TE_E` 사용자가 2명 이상이어야 E 단계를
   넘길 수 있다.
