...
- **컬럼**: 라인 · 목적(기타 목적 세부는 줄바꿈) · MAP 목적(ADI CD 변경은 '해당없음') · 제품 조합 ·
  상신일 · 의뢰자(이름/부서) · 현재 단계 · **범주**(내 개인 마킹 범주 이름, 없으면 '표시 없음').
  - 현재 단계: 단일 행은 `상태 + 단계 텍스트`, 그리드(§3.3.2)는 칸마다 한 줄 `단계명(이름) 상태`
    (해당없음 칸 생략). 중단 요청중·철회 요청중 칩은 글자로 덧붙인다.
  - 범주가 있는 행은 **행 전체를 범주 색으로 칠하고**, 어두운 색이면 글자를 흰색으로 바꾼다.
