...
- **R 합의 후(병렬)**: 한 의뢰서를 **2행**(+후결자 1행)으로 분리 표시 — path1(P), path2(J/O/E), path3(RA).
  rowSpan으로 좌측 공통열 병합. **(2026-08)** J 가 병렬 단계로 분리되면서 path1 에서 빠져 path2 에 합류했다
  — 행 수는 그대로이고, path1 은 **P(+PV) 만으로 완료 판정**된다(J 대기 여부와 무관).
  - path1 단계 텍스트: 현재 pending인 **P(담당자명)** → 담당자 합의 후 지정된 검토자(PV)가 남아 있으면 **검토자(이름 / 이름...)** → 전부 끝나면 **P·PV 결재자 전원의 '라벨(이름)'을 ` / `로 나열**한다. 이름이 하나도 없으면(레거시 데이터 등) "결재완료"로 대체.
  - path1 done 조건(2026-08): P pending 없음 AND PV pending 0개. **J 조건은 제거**됐다 — J 는 더 이상 이 경로에 없다.
  - path2 단계 텍스트: pending인 J/O/E를 ` / `로 연결. **J 는 O 와 동일하게 진행 중 담당자 이름을 표시하지 않는다.** **O는 J와 동일한 검토중 방식이라 진행 중엔 담당자 이름을 표시하지 않고**(2026-07 변경, 이전엔 표시했음), **E는 담당자 이름을 표시**한다(둘 다 P/E와 O/J 짝의 비대칭 규칙). E 담당자 합의 후 지정된 검토자(EV)가 남아 있으면 함께 나열. **모두 끝나면 O 포함 실제로 결재한 전원의 '라벨(이름)'을 ` / `로 나열**한다(2026-07 변경).
  - ✅ **경로2 상태점(2026-08)**: J·O(+E)가 한 행에 모여 있고 아직 진행 중(`!done`)이면, `DocTableRow.subStages`에
    J/O/E 각각의 개별 상태(`wait`/`review`/`done`)를 채운다(생성 조건 `!done && jSteps.length > 0 && oStep`
    — E 없는 비 plel 문서는 J·O 두 개). **화면은 `subStages` 가 있으면 `stageText` 대신 상태점만 렌더**해
    같은 정보가 두 번 보이지 않는다(`ApprovalPage`/`HomePage` 공통). `StageDots`(`frontend/src/components/StageDots.tsx`)가
    대표 뱃지 옆에 라벨+점(회색 대기중 / 주황 검토중 / 초록 완료)으로 렌더링해, "O는 아직 대기중인데
    E만 먼저 검토중으로 선점된" 것 같은 중간 상태가 대표 뱃지 하나로는 안 보이던 문제를 보완한다.
    O는 기존 규칙대로 진행 중엔 이름을 표시하지 않는다.
    **`subStages`가 있으면 `ApprovalPage`·`HomePage` 는 `stageText`(문자열) 대신 `StageDots`만 렌더링한다**
    (같은 정보가 문자열과 점으로 두 번 표시되지 않도록 — `row.subStages ? <StageDots/> : <span>{stageText}</span>`).
    둘 다 완료되면(`done`) subStages는 비우고 기존처럼 `namedApprovers` 텍스트로 되돌아간다.
    ApprovalPage·HomePage 둘 다 이 필드를 그대로 렌더링해 두 화면이 동일하게 보인다.
    (경로2에만 적용 — path1/path3/parallel4(MAP 삭제/수정 R+J+P+O 한 행)에는 적용하지 않음, 별도 검토 대상으로 보류.)
  - ✅ **E 상태점 이름 전환(2026-08)**: E는 진행 중일 때 **"현재 공이 누구에게 있는지"** 를 보여준다 —
    검토자(EV)가 아직 지정되지 않았으면 선점한 담당자 이름(예: `MASK(A)`), 담당자가 검토자를
    지정하면(E `approved` + EV `pending` 동시 생성, Case G) 그 순간부터 **아직 합의하지 않은 검토자
    이름**으로 바뀐다(예: `MASK(B)`, 다중이면 `MASK(B / C)`). EV 는 P/PV 와 동일하게 **전원 합의(AND)**
    라 한 명이 합의해도 나머지가 남아 있는 동안은 그 나머지 이름만 계속 보여준다(예: B 합의 후 `MASK(C)`).
    전원 합의가 끝나면 `done` 으로 넘어가 상태점 자체가 사라지고 `namedApprovers` 텍스트(O·E·검토자 전원
    이름 나열)로 표시된다.
  - path3(후결자 RA) 단계 텍스트도 동일하게, 완료 시 미표시가 아니라 **후결자 전원의 '라벨(이름)'을 나열**한다(2026-07 변경).

- ✅ **반려(rejected) 표시(2026-08)**: 반려 문서는 `getDocTableRows` 의 **최우선 분기**에서 단일 행으로
  렌더한다 — **반려된 단계의 `라벨(이름)`**(여러 단계면 ` / ` 연결, 없으면 `-`) + `rejected` 뱃지,
  완료예정 칸은 `-`(`getFinalCompletionDate` 도 반려 시 `-`). 반려 후에도 잔여 `pending` step 이
  남기 때문에, 이 분기가 없으면 아래 R/RV·병렬 분기가 그 잔여 단계를 진행 중으로 오판한다
  (R 단계에서 반려했는데 함께 지정돼 있던 검토자(RV) 단계로 넘어간 것처럼 보이던 버그, §6-8).
...
- J·O·E·P도 검토중 방식으로 전환되어(2026-07 P 포함), 선점 전에는 **대기중**, 선점 후에는 **검토중**으로 표시된다.
  PV/EV(검토자)는 지정 즉시 assignee가 확정되므로 pending 이면 항상 `under_review`로 표시된다(대기중 상태 없음).
...
- ⚠️ **경로2 대표 뱃지 판정 수정(2026-08)**: 예전엔 `resolvePathStatus(p2MainPending[0] ?? evPendingSteps[0], …)`처럼
  O→E 순서상 **먼저 오는 pending 항목 하나만** 보고 뱃지를 정해서, O가 아직 미배정이면 E가 이미 검토중이어도
  뱃지가 '대기중'으로 보이는 문제가 있었다. 지금은 `p2MainPending`·`evPendingSteps`를 통틀어
  **하나라도 `assignee_loginid`가 있으면 검토중**으로 판정한다(경로2 한정 — 다른 경로는 원래부터
  이 문제가 없거나 별도 검토 대상). 개별 O/E 상태는 §3.3 상태점 참고.
...
- **최종 완료예정**(`getFinalCompletionDate`): R 합의 후 max(path0End, path1End, path2End, path3End).
  - path1End = **P.due**.
  - path2End = **max(J.due, O.due, E.due)**.
  - ✅ **(2026-08) 추정치 제거**: 예전엔 J 가 P 완료 후에야 생성돼 J 기한을 `P.due + 4 달력일`로
    추정했고, 영업일이 아니라 백엔드 실제 값과 어긋났다(§6-5 알려진 부정확 항목). 이제 J 는 R 합의
    시점부터 실제 `due_date` 를 가지므로 추정 로직을 삭제하고 실제 값만 쓴다.
...
- **(2026-08) EV 를 OR → AND 로 재전환**: 바로 위 항목의 OR(1명 합의)이 실제 요구사항과 달랐다 —
  ...
  "EV 전원 합의 후 닫힘"), 상세 결재 경로 표시, 프론트 `approvalTable.ts` 의 경로2 상태점(§3.3).
