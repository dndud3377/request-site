...
## 12. 결재 상세페이지 J/O-layer 공유 필터 (2026-08-25, 초기화 2026-09, 개인 색상적용 2026-09)
...
### 12-6. 개인 색상적용 (2026-09)

공유 필터(12-1~12-5)의 확장판이지만 **팀 공유가 아니라 개인용**이다 — 규칙 정의(키워드+색)와
적용 결과 모두 **서버에 저장하지 않고 이 브라우저의 localStorage에만** 저장한다. 다른 사용자에게는
보이지 않으며, 새로고침해도 유지된다(브라우저를 바꾸거나 캐시를 지우면 사라짐).

- **동작**: `plel` 이 있으면 `pp` 셀이 자동으로 노란색(`VALIDATION_CELL_COLOR`)이 되는 기존 동작의
  확장판이다. sp/sd/pp 각각에 키워드를 등록하고 원하는 색을 고르면, "색상적용" 버튼으로 매칭되는
  **필드의 셀만** 그 색으로 칠한다(행 전체가 아니라 매칭된 sp/sd/pp 셀만). 공유 필터와 달리 `st`나
  `new_or_copy` 등 실제 데이터는 전혀 건드리지 않는 순수 시각 효과다.
- **매칭 기준**: 공유 필터와 동일(`sp`/`sd`/`pp` 키워드 부분 일치, 대소문자 무관) —
  `frontend/src/utils/layerColorRules.ts`의 `matchColorFields`. 이미 비활성(`st==='X'`)인 행은
  매칭 대상에서 제외한다(공유 필터의 매칭 제외 규칙과 동일).
- **저장 위치**: `localStorage`
  - 규칙 정의: `layerColorRules_{J|O}` — 브라우저별로 유지되는 개인 규칙 목록.
  - 적용 결과: `layerColorApplied_{J|O}_{문서id}` — `{rowId: {sp?, sd?, pp?}}` 형태로, 문서별로
    분리 저장된다. 회차가 바뀌어도 별도로 무효화하지 않지만, 행 id가 바뀌면 자연히 매칭되지 않는다.
- **노출 조건**: 공유 필터와 동일 조건(`canUseLayerFilter`)을 그대로 재사용한다 — 역할이
  TE_J(J)/TE_O(O)/MASTER 이고, 문서가 `under_review`/`pause` 이며, 해당 단계가 이번 회차에 아직
  합의되지 않았을 때만 "색상적용"/"색상 관리" 버튼이 뜬다.
- **초기화**: 별도 버튼을 만들지 않고 기존 필터 "초기화" 버튼을 그대로 재사용한다. 클릭 시
  (1) 이 문서·테이블의 로컬 색상 적용 상태를 항상 먼저 지우고, (2) 서버에 필터 적용 baseline이
  있을 때만 기존 `reset-layer-filter` API도 호출해 필터 적용분을 상신 시점 값으로 되돌린다.
  즉 필터만 썼든 색상만 썼든 둘 다 썼든, 초기화 버튼 하나로 전부 원복된다. 이 때문에 초기화 버튼
  노출 조건도 "서버 baseline 존재" **또는** "로컬 색상 적용 이력 존재"로 넓혔다
  (`ApprovalPage.tsx` `canResetLayerFilter`).
- **화면**: `PagedDetailView.tsx`의 J-ayer/O-ayer 표 위, 기존 "공유 필터" 툴바 바로 아래에
  "색상적용" 툴바(규칙별 색상 버튼 + "색상 관리" 버튼)를 추가했다. 규칙 관리 모달은 공유 필터의
  `FilterManageModal`을 본뜬 `RequestPage/components/ColorManageModal.tsx`(색상 선택 `<input
  type="color">` 추가)를 쓴다. 이력 조회(`HistoryPage.tsx`)에는 이 props를 넘기지 않아 버튼 자체가
  뜨지 않는다(공유 필터와 동일한 원칙).
- **백엔드 변경 없음**: 이 기능은 서버 모델·API·마이그레이션이 전혀 없다. 순수 프론트엔드
  기능이다.
- **테스트**: `frontend/src/utils/layerColorRules.test.ts` — 필드 매칭(대소문자 무관/부분 일치,
  키워드 없을 때 매칭 안 됨), localStorage 저장/조회(테이블별·문서별 분리, 손상된 JSON일 때 빈 값
  반환).
