...
지정 시 **담당자+검토자 전원 합의**가 끝나야 그 단계가 완료된 것으로 본다. P는 검토자가 없으면 담당자 합의만으로 완료되지만,
...
- ✅ **E 검토중 + 다중 검토자(2026-07)**: E도 P와 동일하게 검토중 선점 후, `approve-step/`(agent='E')에
  ...
  최종 승인 판정 시 E는 **담당자+검토자(EV) 전원 합의**(`_stage_reviewers_complete`)를 요구한다.
...
- ✅ **(2026-08) E 는 2차 검토자 지정이 필수**: MASK 검증은 2인 확인 절차라서 담당자 혼자 합의로
  ...
  강제하면 영구 정지된다. ⓑ 이 배포 이전에 되감겨 EV step 이 살아남은 문서는 `reviewer_loginids`
  없이도 통과한다(살아있는 EV step 존재를 지정으로 간주).
...
- 완료 조건: 담당자 + 지정된 검토자 **전원 합의**가 끝나야 해당 단계가 완료로 간주된다(Case F/G 참고,
  `_stage_reviewers_complete`). 검토자가 하나도 없으면 담당자 합의만으로 즉시 완료 — **P는 지금도
...
- **(2026-08) Validation System 판정 주체를 상신자로 단일화**: 대상/비대상을 정하는 주체는 **상신자 하나**다. MASK(E) 합의 모달의 확정 토글을 제거하고(`approve-step` 의 `validation_system` 수용도 삭제), 결재현황 상세보기 J-layer 탭에 **상신자 본인에게만** 활성화되는 토글을 뒀다(`POST /documents/<id>/validation-system/`). 수정 창은 상신 직후부터 **E 단계(담당자+검토자 전원 합의) 완료 전**까지. E 담당자 합의 후 값이 바뀌면 **E 단계만** `pending` 으로 되감고 사유를 `comment` 에 남긴다 — 반려처럼 새 회차를 돌리지 않는다.
- **(2026-08) 되감기 시 EV step 삭제**: 위 되감기는 그 회차의 **EV(2차 검토자) step 전체(상태 무관)를 삭제**한다 —
  구 동작(`action`만 `pending`으로 되돌리고 지정 이력은 보존)을 이렇게 바꾼 이유는 검토자 필수화(위 Case G)와
  세트이기 때문이다. EV step 을 남겨 두면 그 검토자가 프론트 후보 목록에서 "이미 지정된 검토자"로 제외된 채
  남고, 검토자 없이는 합의를 못 하게 된 상태와 맞물려 **아무도 새로 고를 수 없는 영구 잠금**이 된다.
  삭제하면 그 검토자가 후보로 다시 나타나 재지정이 가능해진다. 해제 사실은 `comment` 에 남기지 않고,
  해제된 검토자에게 취소 메일도 보내지 않는다(의도적 결정 — 되감기 시점의 이력·메일은 고려하지 않음).
  **두 변경은 반드시 함께 배포한다** — 검토자 필수화만 나가면 이미 되감긴 문서가 잠긴다.
