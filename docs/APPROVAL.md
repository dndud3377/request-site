...
완료 판정은 단계마다 다르다 — **E/EV 는 담당자 합의 + EV 중 1명 합의(OR)** 로 끝나고 남은 EV 는 `skip` 으로 닫히며,
**P/PV 는 담당자 + PV 전원 합의(AND)** 다. 검토자가 0명이면 양쪽 모두 담당자 합의만으로 완료된다(하위호환).
...
- 동작: J·O·(E 있으면 **E 담당자 + EV 중 1명**)·(RA 있으면 RA 전원)가 **모두** `approved`일 때만 `status → approved`. 그 전엔 `under_review` 유지.
...
- ✅ **E 검토중 + 다중 검토자(2026-07)**: E도 P와 동일하게 검토중 선점 후, `approve-step/`(agent='E')에
  ...
  최종 승인 판정 시 E는 **담당자 합의 + EV 중 1명 합의**(`_stage_reviewers_complete`, OR)를 요구한다.
...
- 완료 조건: **E/EV 는 담당자 합의 + EV 중 1명 합의(OR)** 로 완료되고 남은 EV 는 `skip` 으로 닫힌다.
  **P/PV 는 담당자 + PV 전원 합의(AND)** 그대로다(Case F/G 참고, `_stage_reviewers_complete`).
...
- **R 합의 후(병렬)**: 한 의뢰서를 **2행**으로 분리 표시 — path1(P/J), path2(O/E). rowSpan으로 좌측 공통열 병합.
  ...
  - ✅ **경로2 상태점(2026-08)**: O·E가 둘 다 존재하고 아직 진행 중(`!done`)이면, `DocTableRow.subStages`에
    O/E 각각의 개별 상태(`wait`/`review`/`done`)를 채운다. `StageDots`(`frontend/src/components/StageDots.tsx`)가
    대표 뱃지 옆에 라벨+점(회색 대기중 / 주황 검토중 / 초록 완료)으로 렌더링해, "O는 아직 대기중인데
    E만 먼저 검토중으로 선점된" 것 같은 중간 상태가 대표 뱃지 하나로는 안 보이던 문제를 보완한다.
    O는 기존 규칙대로 진행 중엔 이름을 표시하지 않고, E는 선점 시점부터 이름을 표시한다.
    둘 다 완료되면(`done`) subStages는 비우고 기존처럼 `namedApprovers` 텍스트로만 표시한다.
...
- J·O·E·P도 검토중 방식으로 전환되어(2026-07 P 포함), 선점 전에는 **대기중**, 선점 후에는 **검토중**으로 표시된다.
  ...
  단 **EV 가 `skip`(다른 검토자가 먼저 합의해 마감)이면 '건너뜀' 회색 배지**로 표시된다
  (`PagedDetailView.stepToInfo` 의 `skipped`).
...
7. ✅ **(2026-07 해결) P·E 검토중(claim) 전환 + 다중 검토자(PV/EV)** — P·E도 지정하기 대신 검토중 방식으로 바꾸고,
   ...
   완료 판정은 `_stage_reviewers_complete` 가 하며 **E/EV 는 OR(1명), P/PV 는 AND(전원)** 다
   (OR 전환은 2026-08 항목 참고). 검토자 없으면 기존과 동일하게 담당자 합의만으로 완료.
...
- **(2026-08) Validation System 판정 주체를 상신자로 단일화**: 대상/비대상을 정하는 주체는 **상신자 하나**다. MASK(E) 합의 모달의 확정 토글을 제거하고(`approve-step` 의 `validation_system` 수용도 삭제), 결재현황 상세보기 J-layer 탭에 **상신자 본인에게만** 활성화되는 토글을 뒀다(`POST /documents/<id>/validation-system/`). 수정 창은 상신 직후부터 **EV 중 1명이 합의하기 전**까지. E 담당자 합의 후 값이 바뀌어도 **되감지 않고**, 값 변경 사실을 E step `comment` 에 note 로 남긴다.
