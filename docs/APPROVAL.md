...
- ✅ **(2026-09)** 칸 순서를 `GRID_SLOT_ORDER = ['RA_FIXED', 'P', 'J', 'O', 'E', 'RA_EXTRA']`로
  변경했다(1행: RFG/PHPSI, 2행: JOB/OVL, 3행: MASK/추가후결자). 예전 순서는 `['P', 'RA_FIXED', 'J', 'E', 'O', 'RA_EXTRA']`(1행: PHPSI/RFG, 2행: JOB/MASK, 3행: OVL/추가후결자)였다.
...
- ✅ **(2026-08) 고정 후결자 칸의 라벨은 `후결자` 가 아니라 `RFG`** 다 — 고정 후결자는 `.env`
  ...
  ⚠️ 그 결과 **1열 1행의 `RFG` 라벨이 문서 유형에 따라 다른 사람을 가리킨다** — 일반·Only MAP 경로는
