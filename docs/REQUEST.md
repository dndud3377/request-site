...
- **검증**: `cd frontend && npx tsc --noEmit` — 신규 에러 0(기존 4건과 동일). `CI=true npx
  ...
  구동해 확인했다.
- **수동 검증 시나리오** (원격 세션이라 브라우저로 직접 확인하지 못했다 — 아래가 검증의 핵심):
  1. [`/request` 새 문서 → 라인·조합법 선택 → 제품 이름·조리법까지 채우고 요청 목적 'ADI CD
     변경' 선택 → '동일 변경 적용 대상' 표까지 진입] → [입력칸은 건드리지 않은 채, 위쪽
     조합법(또는 라인)을 다른 값으로 변경 → 그 조합법의 제품 이름·조리법도 다시 채움] → [기대
     결과: 표 아래 제품 이름 입력칸을 클릭했을 때 **새로 고른 조합법의 제품만** 보이는지 확인
     (이전 조합법의 제품이 섞여 보이면 회귀).]
  2. [조리법 입력칸도 동일하게 새 조합법 기준 후보만 보이는지 확인.]
