...
- **요청**: `PagedDetailView.tsx` "MAP 정보" 탭에서 지도 편차 변경/예외 구역 변경 칩의 실제
  입력값이 라벨과 같은 톤이라 눈에 띄지 않는다는 피드백 → 숫자 값(X/Y 좌표 `um`, 예외 구역
  `mm`)만 진한 색 + 굵게 강조해 한눈에 들어오도록 개선. 다른 모든 필드는 변경하지 않는다.
- **강조 조건(중요)**: 처음엔 "값 문자열에 숫자가 있으면 무조건 강조"로 구현했다가, 예외
  구역이 `'변경 없음'`으로 선택됐는데도 잔여 `ea_value`(예: 기본값)가 문자열에 그대로 붙어
  같이 강조되는 문제가 발견돼 조건을 수정했다. **회차 간 값이 달라졌는지(`changed`, 빨간
  테두리·"이력 확인" 버튼 조건)와는 무관하게**, 지도 편차는 `detail.map_change !==
  MAP_NO_CHANGE`(비C가문) 또는 C가문 리전(값이 `'변경 있음'`인 리전만 애초에 숫자를 생성함),
  예외 구역은 `detail.ea_change !== MAP_NO_CHANGE`일 때만 숫자를 강조한다 — 즉 **"변경
  있음"을 선택하고 값을 입력한 경우에만** 강조되고, "변경 없음"에 값이 남아 있어도 강조되지
  않는다.
- **구현**: 신규 정규식 상수 `NUMERIC_VALUE_PATTERN`(`/\d+(?:\.\d+)?(?:um|mm)/g`)과 헬퍼
  `renderWithHighlightedNumbers()`를 모듈 최상단에 추가. `Chip` 컴포넌트에 선택적 prop
  `highlightNumbers?: boolean`를 추가해 true일 때만 값 문자열에서 숫자+단위 토큰을 찾아
  `<span style={{ color: '#dc3545', fontWeight: 800 }}>`(X표시 삭제 안내 문구·변경 이력
  테두리와 동일한 빨강)로 감싼다. 지도 편차(`request.map`)·예외 구역(`request.ea_change`) 두
  `Chip` 호출부에서 위 조건으로 계산한 `mapHighlight`/`eaHighlight`를 `highlightNumbers`로
  넘기고, 그 외 `Chip` 사용처는 전혀 건드리지 않았다.
- **범위 밖(의도적으로 유지)**: CLONE/EXISTING처럼 `PlaceholderChip`("없음")으로 표시되는
  잠긴 값, `changed`(빨간 테두리 + "이력 확인" 버튼) 로직·`buildMapValue`/`buildEaValue`의
  값 생성 로직 자체는 손대지 않았다. 이력 확인 모달(`FieldHistoryModal`)의 회차별 비교 표는
  이번 변경 범위 밖이라 숫자 강조가 적용되지 않는다.
- **영향 파일**: `frontend/src/components/PagedDetailView.tsx` (1개 파일).
- **검증**: `npx tsc --noEmit` — 신규 에러 0(기존 4건은 이번 변경 파일과 무관한 `Set` es5
  순회 3건 + `GuidePage.tsx` i18n strict 키 1건, 변경 전후 동일하게 존재). `CI=true npx
  react-scripts test --watchAll=false` — **11 suites / 290건 전부 통과**(신규 테스트는 추가
  하지 않았다 — 기존 회귀 테스트가 이번 변경으로 깨지지 않는지만 확인). 백엔드 코드는
  변경하지 않아 백엔드 테스트는 실행하지 않았다.
- **수동 검증 시나리오**:
  1. [예외 구역이 `'변경 있음'`으로 선택되고 값이 입력된 요청서 상세 화면 → "MAP 정보" 탭
     이동] → [예외 구역 변경 칩 확인] → [기대 결과: `0.5mm` 같은 숫자+단위만 빨간 굵은
     글씨(`#dc3545`)로 보이고, "변경: 변경 있음" 등 나머지 텍스트는 기존 색 그대로.]
  2. [예외 구역이 `'변경 없음'`으로 선택됐는데 값 칸에 잔여 숫자가 남아 있는 문서] →
     [기대 결과: 숫자가 강조되지 않고 기존 검정 텍스트로만 보여야 한다 — 강조돼 보이면
     버그(이번 수정의 핵심 확인 포인트).]
  3. [지도 편차가 C가문(리전별)이고 상판은 '변경 없음', 하판은 '변경 있음'+값 입력인 문서]
     → [기대 결과: 상판 줄은 숫자가 없어 강조 대상 자체가 없고, 하판 줄의 `X:`/`Y:` 값만
     빨갛게 강조돼야 한다.]
  4. [이전 회차 대비 값이 바뀐 재상신 문서에서 같은 두 칩 확인] → [기대 결과: 숫자 강조와
     별개로 빨간 테두리 + "이력 확인" 버튼이 그대로 뜨고, 버튼을 누르면 회차별 비교 모달이
     정상적으로 열려야 한다(모달 안 표 값 자체는 숫자 강조 없이 기존 그대로).]
  5. [MAP 목적이 CLONE/EXISTING인 문서에서 같은 탭 확인] → [기대 결과: 지도 편차·예외 구역
     칩이 지금처럼 회색 "없음"(`PlaceholderChip`)으로 보이고 숫자 강조는 나타나지 않는다.]
...
- **버그**: `validateAdiCdRows`(`helpers.ts`)가 STEP_ID/STEP_DESC 가 **둘 다 빈 행**이면서
  **미등록 체크가 안 된 행**을 검증에서 통째로 건너뛰고 있었다(`incompleteIds`·`validCount`
  어디에도 안 잡힘). 표 일부 행만 채우고 나머지(기본 템플릿 5행 포함)를 빈 채로 두면 그 행이
  없는 셈 치고 상신이 그대로 통과됐다 — 미등록 체크를 해야만 "그 STEP 은 없다"는 뜻이 되는데,
  체크 없이도 같은 효과를 내버린 것.
- **수정**: 미등록이 아니면서 두 칸이 모두 빈 행을 이제 **불완전 행**으로 잡는다(한쪽만 채운
  행과 동일 취급) — `validateAdiCdRows`가 이 판정을 그대로 하므로, 이를 재사용하는 셀
  하이라이트(`AdiCdPanel.tsx`)와 상신 게이트(`index.tsx` `addAdiCdGateError`)는 수정 없이
  같은 값을 본다. 미등록 체크된 행은 종전대로 유효 1건으로 집계되며 이 검사에서 제외된다 —
  빈 행을 상신하려면 반드시 미등록을 체크해야 한다.
- **영향 파일**: `frontend/src/pages/RequestPage/helpers.ts`(`validateAdiCdRows`),
  `frontend/src/pages/RequestPage/helpers.test.ts`, `frontend/src/pages/RequestPage/adiCdUnregisteredAndVs.test.tsx`.
- **테스트**: `npx react-scripts test --watchAll=false` — 9 suites / **256건 전부 통과**
  (기존 251건 + 신규 5건: 완전 빈 행이 불완전으로 잡히는 것, 미등록이면 유효로 잡히는 것).
  `npx tsc --noEmit` 은 이번 변경 전과 동일하게 7건(전부 이 변경과 무관한 기존 에러, 신규 0).
- **수동 검증 시나리오**:
  1. [`/request` → 라인·조합법·제품 이름·조리법 입력 → 요청 목적 'ADI CD 변경' 선택] → [기대
     결과: 변경전/변경후 표에 기본 빈 행 5개씩 뜬다.]
  2. [변경전 표 1행에 STEP_ID·STEP_DESC 를 채우고, 2~5행은 그대로 빈 채로 두고 상신 버튼 클릭]
     → [기대 결과: 상신이 **막히고** 2~5행 입력칸에 빨간 오류 테두리(`adi-cd-cell-error`)가
     표시된다 — 종전에는 이 상태로 상신이 그대로 됐었다.]
  3. [같은 상태에서 2~5행의 '미등록' 체크박스를 각각 켜기] → [기대 결과: 오류 테두리가 사라지고
     그 행이 "미등록" 한 칸으로 바뀐다. 변경후 표도 동일하게 처리한 뒤 상신 버튼 클릭하면 정상
     상신된다.]

### 기능 개선 (2026-08-21 — ADI CD 카드: 상세보기 위치 이동 + 노출 조건 좁힘)

- **요청**: 결재 현황/이력에서 "상세 정보" 탭을 볼 때, "동일 변경 적용 대상"·"ADI CD 변경" 두 카드를
  "기본 정보" 카드 바로 아래로 옮긴다(기존엔 흐름도·BEFORE/AFTER 비교 카드보다도 아래, "특이사항"
  바로 위에 있어 한참 스크롤해야 보였다). 또한 노출 조건을 "저장값이 있으면"에서 **요청 목적이
  'ADI CD 변경'일 때만**으로 좁힌다.
- **위치 이동**(`PagedDetailView.tsx`): 두 카드를 "기본 정보"(`approval.section_basic`) 카드
  닫는 태그 직후, "상세 정보"(`approval.section_detail`) 카드보다 앞으로 옮겼다. 흐름도·BEFORE/AFTER
  비교·특이사항 카드는 그대로 아래에 남는다.
- **노출 조건 좁힘**: 기존 `(detail.adi_cd_extra_targets ?? []).length > 0` /
  `(adi_cd_before·after 에 값이 있으면)` 조건에 `isAdiCdChange &&`를 추가했다(이미 선언돼 있던
  `const isAdiCdChange = detail.request_purpose === 'ADI CD 변경'`를 재사용 — 새 변수·import 불필요).
  과거(2026-08-04~19) '기타 목적 > ADI CD 변경'으로 저장돼 `adi_cd_before/after`에 값이 남아 있는
  레거시 문서는 `request_purpose`가 'ADI CD 변경'이 아니므로 이제 이 두 카드가 뜨지 않는다 —
  **저장값 자체는 손대지 않았다**(표시 조건만 바뀜, 마이그레이션 없음).
- **영향 파일**: `frontend/src/components/PagedDetailView.tsx`. 백엔드·마이그레이션 변경 없음.
- **검증(2026-08-21 실행)**: `npx tsc --noEmit` — 작업 전/후 모두 7개(신규 0). `npx eslint
  PagedDetailView.tsx` — 0건(작업 전/후 동일). `CI=true npx react-scripts test --watchAll=false` —
  9 suites / **254건 전부 통과**(이 파일은 기존에도 전용 테스트가 없어 신규/변경 테스트는 추가하지
  않았다 — tsc/eslint + 아래 수동 시나리오로 검증).
- **수동 검증 시나리오** (원격 세션이라 브라우저 확인은 못 했다 — 아래가 검증의 핵심):
  1. [결재 현황 또는 이력 → 'ADI CD 변경'으로 상신된 문서 클릭 → '상세 정보' 탭] → [기대 결과:
     "기본 정보" 카드 바로 다음에 "동일 변경 적용 대상"(추가 대상이 1개 이상일 때만) 카드, 그 다음
     "ADI CD 변경"(변경전/변경후 STEP 표) 카드가 순서대로 보인다. 그 아래에 "상세 정보" 카드가
     이어진다.]
  2. [같은 화면에서 '신규'/'차용' 등 ADI CD 변경이 아닌 문서 클릭] → [기대 결과: "기본 정보" 카드
     바로 다음에 ADI CD 관련 카드가 전혀 없이 바로 "상세 정보" 카드로 이어진다.]
  3. [(있다면) 2026-08-04~19 사이 '기타 목적 > ADI CD 변경'으로 저장된 레거시 문서 클릭] → [기대
     결과: 이제 ADI CD 카드 자체가 뜨지 않는다(과거엔 값이 남아 있으면 떴었음) — 기타 목적 칩과
     결재 경로는 기존과 동일하게 보인다.]
- **잠재 주의사항**: 위 3번 레거시 케이스는 저장값을 지우지 않았으므로, 필요하면 언제든 조건을
  되돌려 다시 노출시킬 수 있다(정보가 유실된 것이 아니라 화면에서만 감춘 것).
...
- **증상**: 참조 요청서 Merge(`Layer 추가/삭제`·`STEPSEQ 변경`·`Overlay 변경`)로 A를 참조해 B를 상신하면,
  A에만 있던 layer가 B에 `layer삭제`로 정확히 표시된다(변경전/변경후 표). 그런데 이후 **B를 참조해 C를
  상신**하면, B 안에 저장돼 있던 이 `layer삭제` 행이 C의 변경전/변경후 표에 `BEFORE=해당 layer →
  AFTER=미등록(삭제됨)`으로 **다시** 나타난다 — C 작성자는 그 layer를 건드리지도 않았는데, 이미 A→B
  단계에서 끝난 삭제 이력이 매 세대(C→D→…)마다 "이번에 새로 삭제됨"처럼 반복 표시된다.
  (`기등록` 라벨이 붙은 행은 이 문제와 무관하다 — Step1 라인/공정 선택 시 자동 채워지는 JOB FILE
  값이 실제 현재 등록 상태를 그대로 반영하므로, 여전히 존재하는 layer는 정상적으로 매칭돼 표에서
  빠진다. 실제 문제는 이미 사라진 layer, 즉 `layer삭제` 라벨이 붙은 행에서만 발생한다.)
- **원인**: 변경전/변경후 비교 함수 `computeBeforeAfter`의 대상 판정 `baTarget`(`helpers.ts:243`)이
  `!disabled && layerid !== ''`만 검사하고 `new_or_copy`(신규/기등록/layer삭제)를 전혀 보지 않았다.
  반면 실제 layer 상태를 재판정하는 `computeLayerMerge`의 `isMergePresent`(`helpers.ts:153-154`)는
  `new_or_copy !== 'layer삭제'`를 "존재" 조건으로 명시하고 있어, 두 함수의 판정 기준이 어긋나 있었다.
  그 결과 참조문서(B)에 저장된 `layer삭제` 행이 다음 참조 비교(B→C)에서도 "비교 대상인 현재 행"으로
  취급돼, cur(C)에 없으면 자동으로 `AFTER=미등록(삭제)` 짝이 다시 만들어졌다.
- **수정**: `helpers.ts`의 `BaComparableRow`에 `new_or_copy?: string`을 추가하고, `baTarget`에
  `r.new_or_copy !== NOC_LAYER_DELETE` 조건을 추가해 `computeLayerMerge`와 동일한 "존재" 기준으로
  맞췄다. 참조문서의 `layer삭제` 행은 이제 ref 쪽 비교 대상에서 제외되므로, 다음 세대 비교에서
  "없던 것"으로 취급된다. 부수 효과로, 그 layer가 이후에 다시 등록되면(cur 쪽에 재등장) 기존
  `added`(BEFORE 미등록) 자동 판정 분기가 그대로 적용되어 "신규"로 정확히 표시된다(별도 특수 처리
  불필요 — 재현 테스트로 확인).
  - `cur`(현재 작성 중인 표) 쪽에도 같은 `baTarget`이 적용되므로, cur 행에 `layer삭제` 라벨이 남아있는
    경우도 함께 제외된다. 실사용에서 cur 쪽에 이 라벨이 남는 경우는 없지만(작성 중인 표는 아직
    `computeLayerMerge`를 거치지 않았거나, 거쳤다면 이미 `merged` 결과로 대체됨) 방어적으로 동일하게
    처리된다.
  - `computeLayerMerge`(3-way 판정, J/O 표 실제 저장값)는 원래부터 `isMergePresent` 기준이 정확해
    손대지 않았다.
- **영향 파일**: `frontend/src/pages/RequestPage/helpers.ts`(`BaComparableRow`, `baTarget`),
  `frontend/src/pages/RequestPage/helpers.test.ts`(신규 테스트 4건).
- **검증(2026-08-19 실행)**: `npx tsc --noEmit` — 신규 에러 0(기존 7건과 동일, 무관한 `Set` es5 순회·
  `GuidePage.tsx` i18n strict 키). `CI=true npx react-scripts test --watchAll=false` — 9 suites /
  **213건 전부 통과**(`helpers.test.ts` 신규 4건 포함: ref의 `layer삭제` 제외·같은 그룹 내 정상 비교·
  재등장 시 `added` 판정·cur 쪽 라벨 방어 케이스). 수정 전 상태로 직접 재현 테스트(임시 파일, 검증 후
  삭제)를 돌려 버그를 실제 출력으로 먼저 확인했고, 수정 후 같은 재현 시나리오로 해소를 재확인했다.
- **수동 검증 시나리오** (원격 세션이라 브라우저 확인은 못 했다 — 아래가 검증의 핵심):
  1. [`/request` → 새 문서 A 작성 → `Layer 추가/삭제` 목적으로 J-layer에 `L1, L2, L3` 등록 → 상신]
     → [기대 결과: A 상신 완료.]
  2. [새 문서 B 작성 → 같은 라인/공정 선택(JOB FILE 자동 채움으로 L1,L2,L3가 표에 채워짐) → 참조
     요청서에서 A 선택 → `L3` 행을 표에서 삭제(또는 비활성화 대신 실제로 지움) → `L4`를 신규로 추가
     → Merge 클릭 → 확인 모달의 건수 확인] → [기대 결과: 변경전/변경후 표에 `L3: 삭제됨`,
     `L4: 신규`만 보인다(L1,L2는 변경 없음이라 표에 안 뜸). → 상신.]
  3. [새 문서 C 작성 → 같은 라인/공정 선택 → 참조 요청서에서 B 선택 → (L1,L2,L4는 JOB FILE 자동
     채움으로 이미 표에 있음, L3는 실제로 없으므로 표에 없음) → C만의 신규 `L5` 추가 → Merge 클릭]
     → [기대 결과(수정 후): 변경전/변경후 표에 `L5: 신규`만 보이고, **`L3`가 다시 "삭제됨"으로
     뜨지 않는다.** (수정 전에는 여기서 `L3: 삭제됨`이 잘못 재등장했다.)]
  4. [3번 이어서, C 표에 라인 J-layer 데이터를 확인한 후 다시 `L3`를 수동으로 추가(재등록 상황 재현)
     → Merge를 다시 계산] → [기대 결과: 변경전/변경후 표에 `L3: 신규`로 뜬다(삭제 이력이 아니라
     새로 추가된 것으로 정확히 판정).]
- **잠재 주의사항**:
  - 이 수정 이전에 이미 B→C 같은 방식으로 상신·저장된 기존 문서가 있다면, 그 문서의 `merge_pairs`에는
    잘못 재등장한 `삭제됨` 항목이 이미 기록돼 있을 수 있다. 이번 수정은 **새로 계산되는 시점부터만**
    적용되며, 과거에 저장된 `additional_notes.detail.merge_pairs` 값을 소급 정정하지는 않는다.
  - `STEPSEQ 변경`·`Overlay 변경` 목적은 J/O-layer가 JOB FILE 자동 채움 대상이 아닐 수 있어(§확인
    필요), 그 경우 `기등록` 라벨이 붙은 행도 이번 수정과 무관하게 여전히 diff에 정상적으로 뜨는지는
    이번 변경 범위에서 별도로 확인하지 않았다.
...
### 추가 변경 이력 (2026-08-17 — 필드별 배지에서 "영상 가이드" 스타일 제거)

- **개요**: 바로 위 항목("필드별 글 가이드 배지 부활")으로 필드 배지를 되살린 직후, 빌트인 데모
  영상이 있는 필드는 필드 옆에도 "영상 가이드" 스타일 배지가 뜨는 것을 확인하고 사용자가 이를
  제거해 달라고 요청했다. "영상 가이드"는 스텝 제목 옆(`StepTourBadge`, 스텝당 1개, 총 5개)에만
  있으면 된다는 요구에 따라, 필드 배지는 **글 가이드가 실제로 있는 기능에만**, **항상 "가이드"
  (글) 스타일로만** 뜨도록 좁혔다.
- **수정** (`RequestPage/index.tsx`):
  - `featureGuideKeys` 로드 시 `GUIDE_DEMO_KEYS`(빌트인 데모 키)를 더 이상 합치지 않는다 —
    `guidesAPI.list({ guide_type: 'feature' })` 로 실제 조회되는 `feature_key`(글 가이드가 있는
    기능)만 담는다.
  - `GuideBadge` 헬퍼에서 `isVideo`(`GUIDE_DEMO_KEYS.includes(fk)`) 분기를 제거 — 항상
    `guide-badge` 클래스 + `guide.guide_btn`("가이드") 라벨로만 렌더한다.
  - 더 이상 쓰이지 않는 `GUIDE_DEMO_KEYS` import 제거.
  - **결과**: 데모만 있고 글 가이드가 없는 필드는 필드 배지 자체가 안 뜬다(빈 배지 방지). 데모+글이
    둘 다 있는 필드는 "가이드"(글) 스타일 배지만 뜨고, 클릭하면 기존과 동일하게 `GuideSlidePanel`이
    열려 데모와 글을 함께 보여준다(패널 내부 로직은 변경하지 않음).
- **영향 파일**: `pages/RequestPage/index.tsx`
- **검증(2026-08-17 실행, 원격 세션·Docker 없이)**:

  | 항목 | 작업 전 | 작업 후 |
  |---|---|---|
  | `npx tsc --noEmit`(`error TS` 수) | 8개 | **8개 그대로**(신규 0 — `frontend/` 밖에서 실행하면 다른 tsc 버전이 잡혀 결과가 달라지므로 반드시 `cd frontend` 후 실행 확인) |
  | `react-scripts test --watchAll=false` | 8 suites / 206건 | **8 suites / 206건 통과** |

  ⚠️ 위 8개의 `error TS`는 원본 베이스 커밋(`4685ab0`)에도 그대로 있던 기존 항목(Set 순회
  downlevelIteration 5건 + i18n 타입 2건 + `approvalTable.test.ts` 1건)이며, 이번 세션의 어떤
  변경과도 무관함을 별도 워크트리로 직접 확인했다.
...
- **수동 검증 시나리오**:
  1. `/guide` → "+가이드 작성" → 유형 "기능" 선택 → 스텝(예: "1. 의뢰 상세") → 기능(예: "라인/조합법")
     선택 → 본문 입력 후 저장.
  2. `/request`(신규 의뢰서 작성) → Step1 진입 → "라인/조합법" 라벨 옆에 파란색 "가이드" 배지가
     보이는지 확인(2026-08-17 수정으로 필드 배지는 데모 유무와 무관하게 항상 이 스타일 하나뿐이다 —
     "영상 가이드" 스타일은 필드에는 뜨지 않는다). 클릭 시 화면 오른쪽에서 슬라이드 패널이 열리고
     방금 작성한 글 내용이 보이면 성공. 스텝 제목(📋 의뢰 상세) 옆의 기존 "영상 가이드" 배지(스텝
     전체 하이라이트 투어)는 그대로 남아있어야 하며, 클릭 시 기존과 동일하게 동작해야 한다(회귀 없음).
...
- **MASK 는 값을 바꾸지 않는다**: `approve-step` 의 `validation_system` 수용은 제거됐다. MASK 가 이견이면 `reject-step` 을 누르는데, **(2026-09) 이제는 다른 단계와 동일하게 즉시 문서를 반려한다** — 결재를 되돌리지 않던 '수정 요청' 특례는 완전히 삭제됐다(§`docs/APPROVAL.md` Case H, 2026-09 항목). 즉 값을 고치려면 재상신(PL 검토부터)해야 하며, 위 "상신자 변경(결재 진행 중)" 수정 창도 반려 시점에 함께 닫힌다.
...
- **주소록 구성원 추가 방식을 loginid 검증 기반으로 전환 (2026-08 추가)**: `/address-book` 화면에서 더 이상 "사이트에 로그인한 적 있는 사용자"만 검색해서 추가할 수 있는 게 아니라, **loginid를 직접 입력해 그 자리에서 서버가 실존 여부를 검증**하고 통과분만 추가하는 방식으로 바뀌었다.
  - **입력**: 구성원 추가 입력창에 loginid를 콤마(`,`)로 구분해 여러 명 입력 후 Enter. 허용 문자는 영문·숫자·`.`·`,`·`@`뿐(그 외 문자는 입력 즉시 필터링됨). `@`가 포함된 조각(이메일 형식 붙여넣기)은 `@` 앞부분만 loginid 후보로 취급한다(도메인 부분 무시). 콤마 없이 여러 이메일을 붙여넣으면 조각이 합쳐져 유실될 수 있음(의도된 제약).
...
- **지정 PL·통보자 후보는 문서의 지역(국내/해외)으로 고른다 (2026-09)**: 상신 모달의 검토자(지정 PL)와
  통보처 후보 목록을 `usersAPI.list('PL')` 고정이 아니라 `plRoleFor(...)`(`types/index.ts`)로 고른다 —
  국내 의뢰서는 `PL`, 해외 의뢰서는 `PL_GL`(해외 제품 담당자) 목록을 부른다.
  - 판정 기준은 **로그인한 사람이 아니라 문서**다(`editDocIsOverseasRef` ← `doc.is_overseas`).
    공유 그룹 멤버가 남의 임시저장을 대신 상신할 때 로그인 역할로 판단하면 지역이 어긋나
    서버 검증(`views._resolve_designated_pls`)에서 400 이 난다. 새 의뢰서는 아직 문서가 없으므로
    작성자 역할로 판정한다.
  - state 가 아니라 ref 인 이유: 이 값은 상신 모달을 여는 클릭 핸들러에서만 읽고 화면에 그리지 않는다
    (`originalRequesterRef` 와 같은 성격).
  - 국내/해외 분리 전체 규칙은 `docs/APPROVAL.md` §1.3 참조.
...
### 버그 수정 (2026-09-17 — 변경 감지: `loaded` 없는 옛 문서 행이 '신규 행 추가'로 오탐되던 문제)

- **증상**(사용자 재현, 문서 id=143): `loaded=false`인 행 21개의 stepseq가 "추가" 21건과 완전히
  일치(레이어 표기 `1` vs `1.0` 정도만 다름) — 실제로는 이미 문서에 있던 정상 행인데 전부 신규로
  오탐됐다.
- **원인**: `layer_drift.py`가 "이 행이 DB 자동채움 출처인가"를 `row.get('loaded')` 하나로만
  판정했다. 그런데 `RequestPage/index.tsx`(1046/1056행)에는 이미 같은 문제를 처리하는 보정이
  있었다 — `const loaded = r.loaded ?? !!r.updated?.trim()`(옛 문서는 `loaded` 필드 자체가 없을
  수 있어 `updated`— 자동채움 함수 2곳에서만 채워지고 수동 행은 항상 빈 문자열 — 로 대신
  판정). 이 보정은 **편집 화면에 불러올 때만** 적용되고 서버에 저장된 `additional_notes` 원본은
  보정되지 않은 값 그대로라서, 백엔드가 원본만 보고 판정하면 옛 문서의 정상 행이 전부
  `saved_by_seq`에서 빠지고 → 마스터 DB에는 있는데 저장엔 없는 것처럼 보여 무조건 '추가'로
  잡혔다(역방향도 깨져 있었다: 그런 행이 마스터에서 실제로 사라져도 애초에 비교 대상이 아니라
  '삭제'로도 안 잡혔다).
- **수정**: `layer_drift.py`에 `_is_loaded(row)` 헬퍼 추가 —
  `bool(row.get('loaded')) or bool((row.get('updated') or '').strip())`. 프론트와 정확히 같은
  기준을 백엔드에도 적용했다.
- **영향 파일**: `backend/api/layer_drift.py`.
- **검증**: 재현 테스트 2건 추가(`$SP/stubs/verify_layer_drift.py`, 프로젝트 밖) —
  ① `loaded=false`+`updated` 있음+마스터와 내용 동일 → 감지 안 됨(수정 확인) ②같은 조건에서
  마스터에 그 stepseq가 없어지면 이번엔 '삭제'로 정상 감지됨(역방향도 함께 수정 확인). CLAUDE.md
  §1-1 절차로 `manage.py test api verify_layer_drift` — **549건 전부 통과**(기존 스케줄러
...
- **수동 검증 시나리오 — 실제 브라우저로 확인 완료 (2026-09-22)**: 원격 세션이라 Docker/MySQL은
  못 쓰지만, sqlite + `AUTH_MODE=dev`(운영 DB 아님, 로컬 파일 DB에 이 검증용으로 직접 시드한
  마스터 데이터)로 백엔드(`manage.py runserver`)·프론트(`npm start`, `REACT_APP_AUTH_MODE=dev`)를
  둘 다 띄우고 Playwright(headless Chromium)로 실제 화면을 조작해 아래 1~3, 5를 스크린샷과 함께
  직접 확인했다(4, 6은 기존 동작 유지 항목이라 이번엔 자동화 테스트로만 재확인).
  1. [`/request` 새 문서 → 라인 선택 → 조합법 입력/선택] → ✅ 확인됨: 제품 이름을 고르기 전에
     조리법 입력칸 드롭다운을 열면 `["X","Y"]`가 바로 보임(스크린샷 `21_top_process_id_broad_dropdown`).
  2. [1번 이어서, 제품 이름은 비워둔 채 조리법 칸에 `Y` 입력] → ✅ 확인됨: 정상 선택되고, 이어서
     제품 이름 칸 드롭다운이 `["P1"]`로 좁혀짐(P2는 조리법 Y가 없어 빠짐 — 스크린샷
     `22_top_product_narrowed_by_Y`).
  3. [2번 이어서, `P1`을 제품 이름으로 선택] → ✅ 확인됨: 조리법 입력칸 값이 `Y`로 그대로 유지됨
     (스크린샷 `23_after_choosing_P1_process_id_still_Y`).
  4. [새 문서에서 이번엔 순서대로 라인 → 조합법 → 제품 이름을 먼저 선택] → 기존과 동일하게 조리법
     후보가 그 제품 기준으로 좁혀지는지 확인(회귀 없음) — 자동화 테스트(`lineProcessCrossFilter.test.tsx`)로
     확인, 이번 수동 검증에서는 재확인하지 않음.
  5. [4번 이어서, 좁혀진 조리법 후보 중 하나를 선택] → ✅ 확인됨(아래 "ADI CD 대상 패널" 절의
     브라우저 검증에서 `제품 이름=P1 → 조리법=Y` 순서로 동일하게 확인 — 제품 이름이 지워지지
     않음).
  6. [조합법 또는 라인을 다른 값으로 바꾸기] → 제품 이름·조리법이 모두 초기화되고 새 조합법
     기준의 넓은 후보로 다시 채워지는지 확인(기존 동작 유지) — 자동화 테스트로 확인, 이번 수동
     검증에서는 재확인하지 않음.
...
- **검증**: `cd frontend && npx tsc --noEmit` — 신규 에러 0(기존 4건과 동일). `CI=true npx
  ...
  확인했다.
- **수동 검증 시나리오 — 실제 브라우저로 확인 완료 (2026-09-22)**: sqlite + `AUTH_MODE=dev` 로컬
  서버(운영 DB 아님, 이 검증용으로 직접 시드)를 띄우고 Playwright(headless Chromium)로 실제
  화면을 조작해 1, 3을 스크린샷과 함께 직접 확인했다(2, 4는 기존 로직 재사용/단순 CRUD라 이번엔
  자동화 테스트로만 재확인).
  1. [`/request` 새 문서 → 라인·조합법 선택 → 제품 이름에 조리법을 2개 이상 가진 제품(P1: X, Y)을
     선택 → 조리법 중 P2는 안 갖는 값(Y)을 선택 → 요청 목적 'ADI CD 변경' 선택] → ✅ 확인됨:
     '동일 변경 적용 대상' 표 아래 제품 이름 입력칸을 클릭하면 `["P1","P2"]`가 드롭다운에 보임
     (방금 고른 조리법 Y를 안 가진 P2도 정상적으로 보임 — 스크린샷
     `04_adi_cd_draft_product_dropdown`, 사용자에게도 전송).
  2. [1번 이어서, 입력칸에서 제품 이름을 하나 선택] → 조리법 입력칸 후보가 그 제품이 실제로
     가진 조리법으로 좁혀지는지 — `adiCdTargetsCrossFilter.test.tsx`(자동화 테스트)로 확인,
     이번 수동 검증에서는 재확인하지 않음.
  3. [입력칸을 지우고 이번엔 조리법을 먼저 선택(X)] → ✅ 확인됨: 제품 이름 입력칸 후보가 X를
     가진 제품(`["P1","P2"]`, 둘 다 X 보유)으로 나타남(스크린샷
     `05_adi_cd_draft_product_after_pid_X`) — 그중 하나를 선택해도 방금 고른 조리법이 사라지지
     않는 동작은 `adiCdTargetsCrossFilter.test.tsx`의 3번째 테스트로 확인.
  4. [2번 또는 3번 이어서 "추가" 버튼 클릭] → 표에 새 행이 정상 반영되는지 —
     `adiCdTargetsCrossFilter.test.tsx`(자동화 테스트)로 확인, 이번 수동 검증에서는 재확인하지
     않음.

### 버그 수정 (2026-09-22 — ADI CD 대상 패널이 열린 채 라인/조합법을 바꾸면 제품이름·조리법 후보가 갱신되지 않던 stale 버그)

- **배경**: 바로 위 "ADI CD 대상 패널 독립 상호 좁힘" 수정 직후 사용자 요청으로 그 변경 자체를
  다시 검토하다가 발견. 새로 만든 draft 상호 좁힘 effect 2개가 `detail.line`/
  `detail.process_selection`을 의존성 배열에 포함하지 않아, 패널이 열려 있고 draft(입력칸)가
  비어 있는 상태에서 위쪽 라인/조합법을 바꾸면 `''→''`로 실제 문자열 값이 안 바뀌어(재조회를
  유발하던 `setAdiCdTargetDraft({partid_selection:'',process_id:''})` 호출이 매번 새 객체를
  만들어도 원시값 자체는 그대로라 effect 의존성에 변화가 없음) 이전 라인/조합법 기준 옵션이
  그대로 남아 있었다.
- **근거**(재현 테스트, 이번 수정 전 실제 실행 결과): 조합법A(P1: X/Y, P2: X) → 조합법B(P3:
  Z)로 바꾼 뒤 패널의 제품이름 드롭다운을 열면 `['P1','P2']`가 그대로 보였다(기대값 `['P3']`).
- **수정**(`frontend/src/pages/RequestPage/index.tsx`): 위 두 effect의 의존성 배열에
  `detail.line`, `detail.process_selection`을 추가 — 라인/조합법 자체가 바뀌면 draft 값의
  동일 여부와 무관하게 다시 조회한다.
- **영향 파일**: `frontend/src/pages/RequestPage/index.tsx`,
  `frontend/src/pages/RequestPage/adiCdTargetsCrossFilter.test.tsx`(회귀 테스트 1건 추가, 조합법B
  픽스처 추가).
- **검증**: `cd frontend && npx tsc --noEmit` — 신규 에러 0(기존 4건과 동일). `CI=true npx
  react-scripts test --watchAll=false` — 13 suites / **306건**. 3회 중 1회 `requesterResubmitHistory.test.tsx`
  가 전체 스위트를 함께 돌릴 때만 타임아웃으로 실패했으나, 그 파일만 단독 실행(3회 반복)·전체
  스위트 재실행(2회) 모두 정상 통과해 이번 변경과 무관한 기존 타이밍 플레이키로 확인했다
  (이 문서 §4 "ADI CD 대상 패널 독립 상호 좁힘" 항목에도 동일 패턴의 플레이키 사례 있음).
  신규 회귀 테스트는 라인/조합법 변경 시 draft 옵션이 새 범위로 갱신되는지 실제 컴포넌트를
  구동해 확인했다.
- **수동 검증 시나리오 — 실제 브라우저로 확인 완료 (2026-09-22)**: sqlite + `AUTH_MODE=dev` 로컬
  서버(운영 DB 아님, 조합법B/P3를 이 검증용으로 추가 시드)를 띄우고 Playwright로 직접 확인했다.
  1. [`/request` 새 문서 → 라인·조합법(A) 선택 → 제품 이름(P1)·조리법(Y)까지 채우고 요청 목적
     'ADI CD 변경' 선택 → '동일 변경 적용 대상' 표까지 진입] → [입력칸은 건드리지 않은 채, 위쪽
     조합법을 B로 변경 → 그 조합법의 제품 이름(P3)·조리법(Z)도 다시 채움] → ✅ 확인됨: 표 아래
     제품 이름 입력칸을 클릭하면 `["P3"]`만 보임 — 이전 조합법(A)의 P1/P2는 안 남음(스크린샷
     `12_draft_product_dropdown_after_switch_to_B`, 사용자에게도 전송).
  2. [조리법 입력칸도 동일하게 새 조합법 기준 후보만 보이는지 확인] — 같은 effect·같은 의존성
