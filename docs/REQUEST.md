...
### 기능 개선 (2026-09-14 — 상세보기 MAP 정보: 지도 편차·예외 구역 값 안의 숫자(um/mm)만 진하게 강조)
...
- **구현**: 신규 정규식 상수 `NUMERIC_VALUE_PATTERN`(`/\d+(?:\.\d+)?(?:um|mm)/g`)과 헬퍼
  ...
  `<span style={{ color: 'var(--accent-hover)', fontWeight: 800 }}>`로 감싼다. 지도 편차
  (`request.map`)·예외 구역(`request.ea_change`) 두 `Chip` 호출부에만 `highlightNumbers`를
...
- **수동 검증 시나리오**:
  1. [지도 편차·예외 구역 값이 있는 요청서 상세 화면 진입 → "MAP 정보" 탭 이동] → [지도 편차
     변경/예외 구역 변경 칩을 확인] → [기대 결과: `X: 5um`/`Y: 3um`/`값: 0.5mm` 같은 숫자+단위
     부분만 진한 남색 굵은 글씨로 보이고, "변경: 있음"/"사유: …" 등 나머지 텍스트는 기존 색
     그대로 보여야 한다.]
  2. [이전 회차 대비 값이 바뀐 재상신 문서에서 같은 두 칩 확인] → [기대 결과: 숫자 강조와
     별개로 빨간 테두리 + "이력 확인" 버튼이 그대로 뜨고, 버튼을 누르면 회차별 비교 모달이
     정상적으로 열려야 한다(모달 안 표 값 자체는 숫자 강조 없이 기존 그대로).]
  3. [MAP 목적이 CLONE/EXISTING인 문서에서 같은 탭 확인] → [기대 결과: 지도 편차·예외 구역
     칩이 지금처럼 회색 "없음"(`PlaceholderChip`)으로 보이고 숫자 강조는 나타나지 않는다 —
     실패로 보이는 신호: 이 잠긴 문서에서 숫자가 강조돼 보이면 버그.]
