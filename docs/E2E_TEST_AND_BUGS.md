...
| PL_GL (해외 제품 담당자, 2026-09) | `pl_gl1~5` | **GL-A**(작성자), **GL-B**(지정 PL), **GL-C**(2번째 지정 PL) |
...
> GL-B / GL-C 도 마찬가지다. 단 GL 계정끼리만 서로 지정 PL·영업합의자·지정자 변경 대상이 될 수
> 있다 — 국내 PL 을 해외 의뢰서에 지정하거나 그 반대로 지정하면 400 이다(국내/해외 지역 분리,
> `docs/APPROVAL.md` §1.3 참고). 결재현황·홈·이력 조회 자체도 GL 계정은 해외 의뢰서와 본인이
> 올린 문서만 보인다(국내 PL·TE_*·MASTER 는 해외 의뢰서도 종전대로 보이는 단방향 격리).
