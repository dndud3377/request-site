...
| ⚠️ 과거 평문 댓글의 렌더링 안전성 | 2026-08-18 이전에는 `VocComment.content` 가 평문이었고 텍스트로만 렌더링됐다. 이후 댓글 입력이 RichTextEditor 로 바뀌면서 렌더링도 `dangerouslySetInnerHTML` 로 바뀌어, **과거에 저장된 평문 댓글도 HTML 로 해석되어 렌더링된다.** 과거 댓글에 `<`, `>` 등이 포함돼 있었다면 저장형 XSS 가능성이 있다 — 기존 데이터를 이스케이프하는 마이그레이션 적용 여부는 사용자 확인 대기 중(미적용) |
