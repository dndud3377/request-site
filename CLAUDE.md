...
- 이해도가 부족하면 **반드시 즉시** 질문하여 이해도를 높인다.
- 이해도가 충분하면 **반드시** 아래 형식으로 요약 확인을 한다:
  `✅ 이해한 내용 요약: ... 위 내용으로 진행해도 될까요?`
...
## ✅ 규칙 C. 완료 후 검증 및 후속 처리 [필수]

작업이 완료되면 **반드시** 아래를 순서대로 수행한다:

1. **테스트 실행 및 결과 보고** (반드시 실행)
   - Backend: `cd backend && python manage.py test`
   - Frontend: `cd frontend && npm test`
   - 실패 시 수정 후 **반드시** 재실행 결과까지 확인한다.

2. **검증 방법 제공** (반드시 포함)
   - 실행 명령어 또는 브라우저 확인 경로 (`http://localhost:10011`)
   - 확인해야 할 응답값 또는 UI 상태

3. **잠재 버그 및 주의사항 보고** (반드시 포함)
   - 마이그레이션 누락, CORS 설정, 엣지 케이스 등

4. **해당 기능 `docs/*.md` 파일 내용을 반드시 최신화한다.**

5. **`CLAUDE.md`를 다시 읽은 후 반드시 "다음 작업은 무엇인가요?" 라고 묻는다.**

---

## 🔒 규칙 D. 보안 [필수]

- `.env` 파일은 **반드시 읽기만** 한다. 수정 **절대 금지.**
- API 키, 비밀번호, 토큰을 코드에 **반드시 하드코딩하지 않는다.**
...
- 명시적 허락 없이 **반드시** 기존 파일을 삭제하지 않는다.
- 여러 파일을 수정했을 경우 **반드시** 파일별로 개별 커밋한다.
- 새 파일 생성 전 **반드시** 같은 역할의 파일이 이미 존재하는지 확인한다.
- 파일 수정 범위가 클 경우 **반드시** 사용자에게 영향 범위를 먼저 보고한다.
...
- 실행한 테스트 명령어
- 완료/미완료 작업 목록
- 주요 기술 결정사항 (JWT 방식, REST vs GraphQL 등)
...
프론트엔드의 모든 텍스트는 **반드시** i18n 파일을 통해 사용한다. 하드코딩 **절대 금지.**

- 라이브러리: `i18next` 23.10.0 / `react-i18next` 14.1.0
- 설정 파일: `frontend/src/i18n.ts` (언어 감지: localStorage → navigator)
- 위치: `frontend/src/locales/ko.json` (기본) / `frontend/src/locales/en.json`
...
2. 컴포넌트에서 `const { t } = useTranslation();` 후 `{t('키')}` 방식으로 사용한다.
