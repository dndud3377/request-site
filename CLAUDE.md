...
## ⚡ 규칙 0. 모든 작업 시작 전 필수 확인 — 절대 생략 불가

> 🚨 **이 규칙은 세션이 길어지거나 반복 작업이어도 단 한 번도 생략할 수 없다.**

새로운 작업 요청을 받으면 코드 작성 전에 **반드시** 아래 순서를 따른다:

1. **CLAUDE.md를 다시 읽는다.** — 규칙을 숙지한 후 작업을 시작한다.
2. **작업 계획을 먼저 제시하고 사용자의 승인을 받는다.** (→ 규칙 3 참고)
3. **승인을 받기 전까지 코드를 한 줄도 작성하지 않는다.**

```
✋ 계획 없이 코드를 바로 작성하는 것은 규칙 위반이다.
   아무리 작은 작업이라도 계획 제시 → 승인 → 구현 순서를 지킨다.
```
