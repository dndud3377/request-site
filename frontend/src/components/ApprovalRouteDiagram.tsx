 ...
 * 최종 결재 경로: PL(검토) → R(RFG, 담당자→검토자 순차이나 같은 단계라 하나로 표시) →
 *   [경로1: P(PHPSI)] ∥ [경로2: J(JOB)] ∥ [경로3: O(OVL)] ∥ [경로4: E(MASK)] ∥ [후결자] → 완료
 * (2026-08) J(JOB)는 P 뒤 순차 단계에서 독립 병렬 경로로 분리됐다.
 * P/J/O/E는 R 합의 시점에 동시에 생성되는 독립 병렬 단계다(docs/APPROVAL.md).
 * E(MASK)는 plel 존재 시에만, Only MAP 의뢰는 R까지만 진행한다는 조건을 주석으로 안내한다.
 */
  ...
  return (
    <div className="route-diagram" data-tour="approval-route">
      ...
      <div className="route-diagram-flow">
        ...
        <Arrow />
        {/* 담당자(R)→검토자(RV)는 순차지만 같은 R단계라 결재현황 화면과 동일하게 하나로 표시한다 */}
        <Box label={agent('R')} />
        <Arrow />
        ...
        <div className="route-diagram-parallel">
          ...
          <div className="route-diagram-path">
            <span className="route-diagram-path-tag">{t('approval.path_label_4')}</span>
