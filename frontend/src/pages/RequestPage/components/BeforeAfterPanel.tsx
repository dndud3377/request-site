...
import { MERGE_UNREGISTERED_ID } from '../constants';

/**
 * 참조 요청서 Merge 의 변경전/변경후 비교 패널.
 *
 * 표 2개로 구성한다.
 *  ① BEFORE / AFTER — 자동으로 짝지을 수 없어 사용자가 직접 매핑할 항목.
 *     BEFORE 행은 여러 번 선택할 수 있고(1:N), AFTER 행은 적용하면 목록에서 사라진다.
 *  ② 변경전 / 변경후 — 확정된 짝. ✕ 로 해제하면 양쪽 행이 각자의 표로 되돌아간다.
 *
 * 값 계산은 하지 않는다(부모가 넘긴 detail 을 그리기만 한다).
 */
...
export interface BeforeAfterPanelProps {
  ...
  onSelect: (side: 'before' | 'after', id: string) => void;
  onApply: () => void;
...
const BeforeAfterPanel: React.FC<BeforeAfterPanelProps> = ({
  ...
  const kindBadge = (kind: MergePair['kind']) => (
    <span className={`ba-badge ba-badge-${kind}`}>
      {t(`request.ba_kind_${kind}`)}
    </span>
  );

  /** 미매칭 목록 한쪽(BEFORE/AFTER)을 그린다. 첫 행은 항상 '미등록'. */
  const renderPickList = (side: 'before' | 'after', rows: MergeUnmatchedRow[], selected: string | null) => (
  ...
  /** 확정 표의 한쪽 5칸. 값이 없으면(미등록) 5칸을 합쳐 한 줄로 표시한다. */
  const renderPairCells = (info: MergeRowInfo | null, other: MergeRowInfo | null) => {
    if (!info) {
      ...
      <div className="ba-pane">
        <div className="ba-result-scroll">
          <table className="ba-table ba-result-table">
            ...
            <tbody>
              {pairs.map((pair, idx) => (
                <tr key={`${pair.beforeId ?? 'none'}_${pair.afterId ?? 'none'}_${idx}`}>
                  <td>{tableBadge(pair.table)}</td>
                  {renderPairCells(pair.before, pair.after)}
                  {renderPairCells(pair.after, pair.before)}
                  <td>{kindBadge(pair.kind)}</td>
                  <td>
                    <button
                      type="button"
