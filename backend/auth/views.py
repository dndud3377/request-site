...
def get_adfs_public_key():
    ...
    cert_file = getattr(settings, 'OIDC_CERT_FILE_NAME', '***.cer')
...
# ============================================
# Development Only Views (Mock Login)
# ============================================
...
def dev_login_view(request):
    ...
    개발용 Mock 로그인 (password 검사 없음)
    
    요청:
    {
        "username": "RM"  // Mock 사용자 이름
    }
    ...
    if not username:
        return Response(
            {'error': 'username 이 필요합니다.'},
            status=status.HTTP_400_BAD_REQUEST
        )
    
    # DevBackend 로 인증
    from apps.auth.backends.dev import DevBackend
    backend = DevBackend()
    user = backend.authenticate(request, username=username)
    ...
    # 서비스용 JWT 토큰 생성 (개발용)
    service_jwt_secret = getattr(settings, 'SERVICE_JWT_SECRET_KEY', 'dev-secret-key')
    service_jwt_algorithm = getattr(settings, 'SERVICE_JWT_ALGORITHM', 'HS256')
    access_token_lifetime = getattr(settings, 'SERVICE_JWT_ACCESS_TOKEN_LIFETIME', timedelta(hours=1))
    ...
    response.set_cookie(
        ...
        value=access_token,
        httponly=True,
        secure=False,  # 개발용은 HTTP
        samesite='Lax',
        max_age=access_token_lifetime.total_seconds(),
        path='/',
    )
    
    logger.info(f"[DevAuth] Login success: {username}")
    return response


@api_view(['GET'])
@permission_classes([AllowAny])
def dev_users_list(request):
    """
    ...
    응답:
    [
        {"username": "게스트 1", "name": "게스트 1", "role": "NONE", ...},
        ...
    ]
    """
    # 개발 환경인지 확인
    ...
    from apps.auth.backends.dev import DevBackend
    users = DevBackend.get_all_users()
...
@api_view(['GET'])
@permission_classes([AllowAny])
def dev_users_by_role(request, role):
    """
    GET /api/auth/dev/users/<role>/
    역할별 Mock 사용자 목록 반환
    
    응답:
    [
        {"username": "RM", "name": "RM", "role": "PL", ...},
        ...
    ]
    """
    # 개발 환경인지 확인
    ...
    from apps.auth.backends.dev import DevBackend
    users = DevBackend.get_users_by_role(role)
...
def oidc_callback(request):
    ...
    # OIDC 클레임 필드 매핑 (필드명: 설명)
    # 환경 변수 OIDC_CLAIM_FIELDS에서 설정 가능
    CLAIM_FIELDS = getattr(settings, 'OIDC_CLAIM_FIELDS', [
            ('sub', '토큰 Subject'),
            ('iss', '토큰 Issuer'),
            ('aud', '토큰 Audience'),
            ('exp', '만료 시간'),
    ...
    # NOTE: 상세 클레임 로깅 제거 (보안 목적 - 사외 공유용)
    logger.info("[OIDC] Processing OIDC token claims...")
